...
CREATE OR REPLACE FUNCTION "public"."can_user_have_more_matches"("user_id" "uuid") RETURNS boolean
    LANGUAGE "plpgsql"
    AS $$
DECLARE
...
BEGIN
    ...
    SELECT EXISTS (
        SELECT 1 FROM public.user_waitlist 
        WHERE user_id = user_id AND processed = FALSE
    ) INTO is_on_waitlist;
    
    RETURN active_match_count < max_matches AND NOT is_on_waitlist;
END;
$$;


ALTER FUNCTION "public"."can_user_have_more_matches"("user_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."check_match_cap"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
BEGIN
...
ALTER FUNCTION "public"."check_match_cap"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."check_user_match_limit"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
DECLARE
    user_a_active_count INTEGER;
    user_b_active_count INTEGER;
    max_matches INTEGER := 3;
    user_a_email TEXT;
    user_b_email TEXT;
BEGIN
    -- Get active match count for user_a
    SELECT COUNT(*) INTO user_a_active_count
    FROM public.matches
    WHERE (user_a = NEW.user_a OR user_b = NEW.user_a)
    AND status = 'active';
    
    -- Get active match count for user_b
    SELECT COUNT(*) INTO user_b_active_count
    FROM public.matches
    WHERE (user_a = NEW.user_b OR user_b = NEW.user_b)
    AND status = 'active';
    
    -- Get emails for error messages
    SELECT email INTO user_a_email FROM auth.users WHERE id = NEW.user_a;
    SELECT email INTO user_b_email FROM auth.users WHERE id = NEW.user_b;
    
    -- Check if either user is at or over limit
    IF user_a_active_count >= max_matches AND user_b_active_count >= max_matches THEN
        -- Both users are at limit
        INSERT INTO public.user_waitlist (user_id) VALUES (NEW.user_a) ON CONFLICT (user_id) DO NOTHING;
        INSERT INTO public.user_waitlist (user_id) VALUES (NEW.user_b) ON CONFLICT (user_id) DO NOTHING;
        RAISE EXCEPTION 'Both users % and % have % active matches. Added to waitlist.', 
            user_a_email, user_b_email, max_matches;
    ELSIF user_a_active_count >= max_matches THEN
        -- Only user_a is at limit
        INSERT INTO public.user_waitlist (user_id) VALUES (NEW.user_a) ON CONFLICT (user_id) DO NOTHING;
        RAISE EXCEPTION 'User % already has % active matches. Added to waitlist.', 
            user_a_email, user_a_active_count;
    ELSIF user_b_active_count >= max_matches THEN
        -- Only user_b is at limit
        INSERT INTO public.user_waitlist (user_id) VALUES (NEW.user_b) ON CONFLICT (user_id) DO NOTHING;
        RAISE EXCEPTION 'User % already has % active matches. Added to waitlist.', 
            user_b_email, user_b_active_count;
    END IF;
...
ALTER FUNCTION "public"."check_user_match_limit"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."filter_potential_matches"("user_id" "uuid", "radius_meters" integer DEFAULT 5000) RETURNS TABLE("match_id" "uuid", "full_name" "text", "username" "text", "bio" "text", "avatar_url" "text", "interests" "text"[], "location" "public"."geography", "fade_progress" double precision)
    LANGUAGE "plpgsql"
    AS $$
BEGIN
    RETURN QUERY
...
ALTER FUNCTION "public"."filter_potential_matches"("user_id" "uuid", "radius_meters" integer) OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."process_waitlist"() RETURNS integer
    LANGUAGE "plpgsql"
    AS $$
DECLARE
...
BEGIN
    FOR waitlist_user IN 
        SELECT * FROM public.user_waitlist 
        WHERE processed = FALSE
        ORDER BY added_at
    LOOP
        -- Check if user now has capacity
        IF public.can_user_have_more_matches(waitlist_user.user_id) THEN
            -- Mark as processed
            UPDATE public.user_waitlist
            SET processed = TRUE, processed_at = NOW()
            WHERE id = waitlist_user.id;
            
            processed_count := processed_count + 1;
            
            -- Here you might want to notify the user they can now create matches
            -- For example, using Supabase's realtime functionality
        END IF;
    END LOOP;
...
ALTER FUNCTION "public"."process_waitlist"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."update_fade_progress"() RETURNS "void"
    LANGUAGE "plpgsql"
    AS $$
BEGIN
...
ALTER FUNCTION "public"."update_fade_progress"() OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."dates" (
    ...
    CONSTRAINT "dates_status_check" CHECK (("status" = ANY (ARRAY['pending'::"text", 'confirmed'::"text", 'completed'::"text"])))
);
...
CREATE TABLE IF NOT EXISTS "public"."feedback_reviews" (
    ...
    "feedback" "text",
    "created_at" timestamp with time zone DEFAULT "now"(),
    CONSTRAINT "feedback_reviews_score_check" CHECK ((("score" >= 1) AND ("score" <= 5)))
);


ALTER TABLE "public"."feedback_reviews" OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."interactions" (
    ...
    "notes" "text",
    "created_at" timestamp with time zone DEFAULT "now"(),
    CONSTRAINT "interactions_interaction_type_check" CHECK (("interaction_type" = ANY (ARRAY['match'::"text", 'message'::"text", 'unmatch'::"text", 'date'::"text", 'review'::"text"])))
);
...
CREATE TABLE IF NOT EXISTS "public"."matches" (
    ...
    "fade_progress" double precision DEFAULT 0.0,
    "last_interaction" timestamp with time zone,
    "created_at" timestamp with time zone DEFAULT "now"(),
    CONSTRAINT "matches_status_check" CHECK (("status" = ANY (ARRAY['active'::"text", 'fading'::"text", 'closed'::"text"])))
);
...
CREATE TABLE IF NOT EXISTS "public"."prompts" (
    ...
    "allows_custom_response" boolean DEFAULT true,
    "created_at" timestamp with time zone DEFAULT "now"(),
    CONSTRAINT "prompts_category_check" CHECK (("category" = ANY (ARRAY['Fun & Lighthearted'::"text", 'Flirty & Playful'::"text", 'Deep & Thought-Provoking'::"text", 'Travel & Adventure'::"text", 'Pop Culture & Interests'::"text", 'Hypotheticals & Would You Rather'::"text"])))
);
...
CREATE TABLE IF NOT EXISTS "public"."user_lifestyle" (
    ...
    "user_id" "uuid",
    "has_kids" boolean,
    "open_to_others_with_kids" boolean,
    "smokes" boolean,
    "drinks" boolean,
    "uses_cigarettes" boolean,
    "sexual_orientation" "text",
    "interested_in" "text"[],
    "created_at" timestamp with time zone DEFAULT "now"(),
    CONSTRAINT "user_lifestyle_sexual_orientation_check" CHECK (("sexual_orientation" = ANY (ARRAY['Heterosexual'::"text", 'Homosexual'::"text", 'Bisexual'::"text", 'Pansexual'::"text", 'Asexual'::"text", 'Queer'::"text", 'Other'::"text", 'Prefer not to say'::"text"])))
);


ALTER TABLE "public"."user_lifestyle" OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."user_prompt_answers" (
    "id" "uuid" DEFAULT "extensions"."uuid_generate_v4"() NOT NULL,
    "user_id" "uuid",
    "prompt_id" "uuid",
...
ALTER TABLE "public"."user_prompt_answers" OWNER TO "postgres";
...
ALTER TABLE "public"."user_vibes" OWNER TO "postgres";
...
ALTER TABLE "public"."user_waitlist" OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."users" (
    ...
    CONSTRAINT "user_must_be_18" CHECK ((("date_of_birth" IS NULL) OR ("date_of_birth" <= (CURRENT_DATE - '18 years'::interval)))),
    CONSTRAINT "users_gender_check" CHECK (("gender" = ANY (ARRAY['Woman'::"text", 'Man'::"text", 'Non-binary'::"text", 'Transgender'::"text", 'Agender'::"text", 'Genderqueer'::"text", 'Genderfluid'::"text", 'Two-Spirit'::"text", 'Prefer not to say'::"text", 'Prefer to self-describe'::"text"])))
);
...
ALTER TABLE ONLY "public"."matches"
    ADD CONSTRAINT "unique_pair" UNIQUE ("user_a", "user_b");
...
ALTER TABLE ONLY "public"."user_prompt_answers"
    ADD CONSTRAINT "user_prompt_answers_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."user_vibes"
    ADD CONSTRAINT "user_vibes_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."user_waitlist"
    ADD CONSTRAINT "user_waitlist_pkey" PRIMARY KEY ("id");
...
CREATE INDEX "user_embeddings_embedding_idx" ON "public"."user_embeddings" USING "ivfflat" ("embedding" "public"."vector_cosine_ops") WITH ("lists"='100');



CREATE OR REPLACE TRIGGER "enforce_match_cap" BEFORE INSERT ON "public"."matches" FOR EACH ROW EXECUTE FUNCTION "public"."check_match_cap"();



CREATE OR REPLACE TRIGGER "enforce_match_limit" BEFORE INSERT ON "public"."matches" FOR EACH ROW EXECUTE FUNCTION "public"."check_user_match_limit"();

ALTER TABLE "public"."matches" DISABLE TRIGGER "enforce_match_limit";
...
ALTER TABLE ONLY "public"."dates"
    ADD CONSTRAINT "dates_proposed_by_fkey" FOREIGN KEY ("proposed_by") REFERENCES "public"."users"("id");



ALTER TABLE ONLY "public"."feedback"
    ADD CONSTRAINT "feedback_date_id_fkey" FOREIGN KEY ("date_id") REFERENCES "public"."dates"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."feedback_reviews"
    ADD CONSTRAINT "feedback_reviews_match_id_fkey" FOREIGN KEY ("match_id") REFERENCES "public"."matches"("id");



ALTER TABLE ONLY "public"."feedback_reviews"
    ADD CONSTRAINT "feedback_reviews_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "public"."users"("id");
...
ALTER TABLE ONLY "public"."interactions"
    ADD CONSTRAINT "interactions_partner_id_fkey" FOREIGN KEY ("partner_id") REFERENCES "public"."users"("id") ON DELETE CASCADE;
...
ALTER TABLE ONLY "public"."user_lifestyle"
    ADD CONSTRAINT "user_lifestyle_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "public"."users"("id") ON DELETE CASCADE;
...
ALTER TABLE ONLY "public"."user_prompt_answers"
    ADD CONSTRAINT "user_prompt_answers_prompt_id_fkey" FOREIGN KEY ("prompt_id") REFERENCES "public"."prompts"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."user_prompt_answers"
    ADD CONSTRAINT "user_prompt_answers_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "public"."users"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."user_vibes"
    ADD CONSTRAINT "user_vibes_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "public"."users"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."user_vibes"
    ADD CONSTRAINT "user_vibes_vibe_id_fkey" FOREIGN KEY ("vibe_id") REFERENCES "public"."vibes"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."user_waitlist"
    ADD CONSTRAINT "user_waitlist_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "auth"."users"("id");



CREATE POLICY "Authenticated can view dates" ON "public"."dates" FOR SELECT TO "authenticated" USING (true);
...
CREATE POLICY "User can edit own profile" ON "public"."users" FOR UPDATE TO "authenticated" USING (("auth"."uid"() = "id"));
...
CREATE POLICY "User can view own profile" ON "public"."users" FOR SELECT TO "authenticated" USING (("auth"."uid"() = "id"));
...
GRANT ALL ON FUNCTION "public"."can_user_have_more_matches"("user_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."can_user_have_more_matches"("user_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."can_user_have_more_matches"("user_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."check_match_cap"() TO "anon";
GRANT ALL ON FUNCTION "public"."check_match_cap"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."check_match_cap"() TO "service_role";



GRANT ALL ON FUNCTION "public"."check_user_match_limit"() TO "anon";
GRANT ALL ON FUNCTION "public"."check_user_match_limit"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."check_user_match_limit"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."filter_potential_matches"("user_id" "uuid", "radius_meters" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."filter_potential_matches"("user_id" "uuid", "radius_meters" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."filter_potential_matches"("user_id" "uuid", "radius_meters" integer) TO "service_role";
...
GRANT ALL ON FUNCTION "public"."process_waitlist"() TO "anon";
GRANT ALL ON FUNCTION "public"."process_waitlist"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."process_waitlist"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."update_fade_progress"() TO "anon";
GRANT ALL ON FUNCTION "public"."update_fade_progress"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."update_fade_progress"() TO "service_role";
...
GRANT ALL ON TABLE "public"."feedback_reviews" TO "anon";
GRANT ALL ON TABLE "public"."feedback_reviews" TO "authenticated";
GRANT ALL ON TABLE "public"."feedback_reviews" TO "service_role";
...
GRANT ALL ON TABLE "public"."user_lifestyle" TO "anon";
GRANT ALL ON TABLE "public"."user_lifestyle" TO "authenticated";
GRANT ALL ON TABLE "public"."user_lifestyle" TO "service_role";
...
GRANT ALL ON TABLE "public"."user_prompt_answers" TO "anon";
GRANT ALL ON TABLE "public"."user_prompt_answers" TO "authenticated";
GRANT ALL ON TABLE "public"."user_prompt_answers" TO "service_role";



GRANT ALL ON TABLE "public"."user_vibes" TO "anon";
GRANT ALL ON TABLE "public"."user_vibes" TO "authenticated";
GRANT ALL ON TABLE "public"."user_vibes" TO "service_role";



GRANT ALL ON TABLE "public"."user_waitlist" TO "anon";
GRANT ALL ON TABLE "public"."user_waitlist" TO "authenticated";
GRANT ALL ON TABLE "public"."user_waitlist" TO "service_role";
